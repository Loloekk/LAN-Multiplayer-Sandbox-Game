...
public class MoveService {
    ...
    public static void movePlayer(PhysicalPlayer player, Direction direction) {
        ...
        body.applyLinearImpulse(MoveService.getMoveImpulse(direction), 0, body.getWorldCenter().x, body.getWorldCenter().y, true);
    }
    ...
    public void jumpPlayer(PhysicalPlayer player) {
        ...
        if (body.getLinearVelocity().y == 0) { // Only jump if on the ground
            body.applyLinearImpulse(0, JUMP_IMPULSE, body.getWorldCenter().x, body.getWorldCenter().y, true);
        }
    }
}
